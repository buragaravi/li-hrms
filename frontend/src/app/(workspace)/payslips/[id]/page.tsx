...
/**
 * Render the payslip detail page, fetch payroll data by route id, and provide an executive PDF export.
 *
 * The component fetches payroll details on mount (using the route `id`), displays loading/error states,
 * shows employee, attendance, earnings and deductions information, and allows downloading a formatted PDF of the payslip.
 *
 * @returns The React element for the payslip detail page.
 */
...
/**
 * Renders a compact two-line label/value row used in detail grids.
 *
 * @param label - The uppercase label text displayed above the value
 * @param value - The value to display; rendered as provided and truncated if too long
 * @returns A JSX element containing the labeled value row
 */
...
/**
 * Renders a compact status card showing a short label and a prominent value with selectable color and optional highlight.
 *
 * @param label - Short uppercase label displayed above the value
 * @param value - Value displayed prominently; falsy values render as `0`
 * @param color - Visual theme for the card; supported keys: `slate`, `indigo`, `rose`, `emerald`, `amber`, `blue`
 * @param highlight - When true, applies an emphasized (highlighted) styling instead of the standard color theme
 * @returns A status card React element containing the label and value with the chosen styling
 */
...
/**
 * Renders a single salary line displaying a label and a formatted INR amount, omitting zero-value rows except for certain required labels.
 *
 * @param label - The text label shown on the left (e.g., "Basic Salary", "Attendance Deduct").
 * @param value - The numeric amount to display; formatted as `₹` with two decimal places and Indian grouping.
 * @param isDeduction - If `true`, formats the amount as a deduction (prefixes with `-` and applies deduction styling).
 * @returns The rendered row element, or `null` when the row is intentionally omitted for zero values.
 */
